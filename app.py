...
if st.button("Predict"):
    ...
    st.markdown('<div style="background-color:#f0f0f0; border-radius:10px; padding:10px; margin-top:20px; height:200px; overflow:auto;"><h3>Input Data:</h3>' + input_data_model.to_html(index=False, justify='center') + '</div>', unsafe_allow_html=True)
    ...
    predicted_price = round(car_price[0])


    st.markdown('<div style="background-color:#f0f0f0; border-radius:10px; padding:10px; margin-top:20px; height:200px; overflow:auto;"><h3>Preprocessed Data:</h3>' + input_data_model.to_html(index=False, justify='center') + '</div>', unsafe_allow_html=True)
    
    st.subheader("Prediction Result:")
    st.markdown(f"Predicted Car Price: **{predicted_price} INR**")
